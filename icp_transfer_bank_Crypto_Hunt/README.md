...
dfx build backend   # produce WASM & .did
...
# basic smoke tests
dfx canister call backend addToGoldPot '(100_000_000)'
dfx canister call backend getGoldPot  '()'
Upgrade-safe State (Pots)
Version 1 introduces a version-tagged snapshot:

rust
Copy
Edit
ic_cdk::storage::stable_save((STATE_VERSION, state))
On pre-upgrade we persist (u8 version, State).

On post-upgrade we first try to restore the tagged tuple;
if that fails we fall back to the legacy (StateV0,) (no pots).

New fields are annotated with #[serde(default)], so future, additive schema changes never panic.

Implication: The silver_pot_e8s and gold_pot_e8s values are no longer reset to zero when the canister code is upgraded.

Upgrade Procedure
bash
Copy
Edit
# 0) (optional) make sure pots are > 0
dfx canister call backend getTotalPot '()'

# 1) update your Git repo & build
git pull
dfx build backend

# 2) upgrade in place
dfx canister install backend --mode upgrade
# ↳  the post_upgrade() hook restores the previous pots automatically

# 3) verify
dfx canister call backend getTotalPot '()'   # jackpot unchanged
...
Edit
# Only .did
cargo run --bin ic-wasm -- candid backend.wasm -o backend.did

# Pretty-print the interface to console
didc print backend.did
If you just built with dfx build backend the .did file is already in
target/wasm32-unknown-unknown/release/backend.did.
